...
class InputActivityEventService {
  ...
  Future<List<TrackedInputEvent>> listEventsPage({
    DateTime? start,
    DateTime? end,
    String? processName,
    String? category,
  ...
  _WhereParts _buildWhereClause({
    ...
    if (trimmedCategory != null && trimmedCategory.isNotEmpty) {
      clauses.add('${qualifiedPrefix}category = ?');
      variables.add(Variable<String>(trimmedCategory));
    }

    final trimmedEventKind = eventKind?.trim();
    if (trimmedEventKind != null && trimmedEventKind.isNotEmpty) {
      clauses.add('${qualifiedPrefix}event_kind = ?');
      variables.add(Variable<String>(trimmedEventKind));
    }

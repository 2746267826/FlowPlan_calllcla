...
class TaskRepository {
  ...
  Future<List<TaskItem>> listAllVisible() async {
    final query = _db.select(_db.taskItems).join([
      innerJoin(
        _db.taskLists,
        _db.taskLists.id.equalsExp(_db.taskItems.taskListId),
  ...
  Future<void> updateDtstart(int id, DateTime dtstart) async {
  ...
  Future<void> updateDuration(int id, int durationMinutes) async {
    await (_db.update(_db.taskItems)..where((t) => t.id.equals(id)))
  ...
  Future<void> clearDtstart(int id) async {
    await (_db.update(_db.taskItems)..where((t) => t.id.equals(id)))
  ...
  Future<int> delete(
    ...
    if (audit && deleted > 0 && before != null) {
      ...
      await _syncWriteRecorder?.recordDelete(
        objectType: SyncObjectType.taskItem.key,
        localId: before.id.toString(),
        uid: before.uid,
        payload: before.toJson(),
      );
    }
    return deleted;
  }

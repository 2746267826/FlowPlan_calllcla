...
final _taskWorkLogsProvider =
    FutureProvider.family<List<TaskWorkLog>, int>((ref, taskId) {
  return ref.watch(activityFusionRepositoryProvider).listTaskWorkLogsForTask(taskId);
});
...
class TaskTrackerEvidenceSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final workLogsAsync = ref.watch(_taskWorkLogsProvider(taskId));
          ...
          child: Column(
            ...
            children: [
              ...
              if (records.isEmpty)
                ...
                const SizedBox(height: 18),
                _TaskActualWorkPanel(workLogsAsync: workLogsAsync),
                const SizedBox(height: 18),
                Text(
                  '\u5173\u8054\u5de5\u4f5c\u4f1a\u8bdd',
...
class _TaskActualWorkPanel extends StatelessWidget {
  const _TaskActualWorkPanel({required this.workLogsAsync});

  final AsyncValue<List<TaskWorkLog>> workLogsAsync;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        workLogsAsync.when(
          loading: () => _surfaceMessage('正在读取任务实际投入...', context),
          error: (error, _) => _surfaceMessage(
            '任务实际投入读取失败：$error',
            context,
            isError: true,
            ...
            return Column(
              ...
              children: [
                Wrap(
                  ...
                  runSpacing: 8,
                  children: [
                    _Tag(text: '已确认 ${_formatMinutes(confirmedMinutes)}', highlighted: true),
                    if (candidateMinutes > 0)
                      _Tag(text: '候选 ${_formatMinutes(candidateMinutes)}'),
                  ],
                ),
                const SizedBox(height: 10),
                ...logs.take(6).map((log) => _TaskWorkLogTile(log: log)),
              ],
            );
          },
        ),
...
class _TaskWorkLogTile extends StatelessWidget {
  const _TaskWorkLogTile({required this.log});

  final TaskWorkLog log;

  @override
  Widget build(BuildContext context) {
    final status = log.status == 'confirmed' ? '已确认' : '候选';
    return Container(
      width: double.infinity,
      ...
      child: Row(
        children: [
          Expanded(
            child: Text(
              '${_formatDate(log.startAt)} ${_formatTime(log.startAt)} - ${_formatTime(log.endAt)}',
              style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600),
            ),
          ),
          const SizedBox(width: 8),
          _Tag(
            text: '$status · ${_formatMinutes(log.durationMinutes)}',
            highlighted: log.status == 'confirmed',
          ),
        ],
      ),
    );

...
class TaskScheduleSegmentRepository {
  ...
  Future<void> replaceForTasks({
    ...
    final existingByTaskId = <int, List<TaskScheduleSegment>>{};
    for (final id in ids) {
      final existingSegments = await getByTaskId(id);
      existingByTaskId[id] = existingSegments;
      before[id.toString()] =
          existingSegments.map((segment) => segment.toJson()).toList();
    }

    await _db.transaction(() async {
      for (final id in ids) {
        for (final segment in existingByTaskId[id] ?? const <TaskScheduleSegment>[]) {
          await _syncWriteRecorder?.recordDelete(
            objectType: SyncObjectType.taskScheduleSegment.key,
      ...
      for (final segment in segments) {
        ...
        await _syncWriteRecorder?.recordCreate(
          objectType: SyncObjectType.taskScheduleSegment.key,
          localId: row.read<int>('id').toString(),
          payload: segment.toJson(),
        );
      }
    });

...
class CalendarBooksRepository {
  ...
  Future<int> createEventCalendar(
    ...
    if (audit) {
      ...
      if (created != null) {
        final snapshot = await _eventCalendarSnapshot(created);
        ...
        await _syncWriteRecorder?.recordCreate(
          objectType: SyncObjectType.calendarBook.key,
  ...
  Future<int> createTaskList(
    ...
    if (audit) {
      ...
      if (created != null) {
        final snapshot = await _taskListSnapshot(created);
        await _recordContainerOperation(
          actor: actor,
          action: action,
        ...
        await _syncWriteRecorder?.recordCreate(
          objectType: SyncObjectType.taskList.key,
          localId: created.id.toString(),
          payload: snapshot,
        );
      }

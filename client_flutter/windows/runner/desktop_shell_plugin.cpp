...
void DesktopShellPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "openPath") {
    const std::wstring path =
        Utf8ToWide(ReadStringArgument(method_call, "path", ""));
    result->Success(flutter::EncodableValue(OpenPath(path)));
    return;
  }

  if (method_call.method_name() == "revealPath") {
    const std::wstring path =
        Utf8ToWide(ReadStringArgument(method_call, "path", ""));
...
bool DesktopShellPlugin::OpenPath(const std::wstring& path) const {
  if (path.empty()) {
    return false;
  }
...
bool DesktopShellPlugin::RevealPath(const std::wstring& path) const {
  if (path.empty()) {
    return false;
  }

...
ALTER TABLE devices
  ...
  ADD COLUMN IF NOT EXISTS runtime_platform text,
  ADD COLUMN IF NOT EXISTS network_type text,
  ADD COLUMN IF NOT EXISTS sync_pending_count integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS sync_failed_count integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS open_conflict_count integer NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS file_storage_objects (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  file_item_id uuid REFERENCES file_items(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS sync_objects_activity_payload_time_idx
  ON sync_objects(user_id, (payload->>'startTime'))
  WHERE object_type IN ('activity_record', 'activity_records')
    AND deleted_at IS NULL
    AND payload ? 'startTime';

CREATE INDEX IF NOT EXISTS sync_objects_input_payload_time_idx
  ON sync_objects(user_id, (payload->>'timestamp'))
  WHERE object_type IN ('tracked_input_event', 'tracked_input_events')
    AND deleted_at IS NULL
...
ALTER TABLE admin_remote_configs
  ADD COLUMN IF NOT EXISTS scope text NOT NULL DEFAULT 'user.preference',
...
ALTER TABLE ai_operation_drafts
...
ALTER TABLE activity_segments
  ...
  ADD COLUMN IF NOT EXISTS merge_method text NOT NULL DEFAULT 'rule',
  ADD COLUMN IF NOT EXISTS matched_task_id text,
  ADD COLUMN IF NOT EXISTS matched_event_id text;

ALTER TABLE activity_interpretations
  ADD COLUMN IF NOT EXISTS title text,
  ADD COLUMN IF NOT EXISTS interpreted_type text NOT NULL DEFAULT 'unknown',
  ADD COLUMN IF NOT EXISTS matched_task_id text,
  ADD COLUMN IF NOT EXISTS matched_event_id text,
  ADD COLUMN IF NOT EXISTS matched_folder_id text,
  ADD COLUMN IF NOT EXISTS model_used text NOT NULL DEFAULT 'rule',
...
ALTER TABLE actual_activity_logs
  ADD COLUMN IF NOT EXISTS duration_seconds integer NOT NULL DEFAULT 0;
...
ALTER TABLE file_transfer_sessions
  ADD COLUMN IF NOT EXISTS source_device_id uuid REFERENCES devices(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS target_device_id uuid REFERENCES devices(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS source_node_id uuid REFERENCES file_nodes(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS target_root_id uuid REFERENCES file_roots(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS transfer_type text,
  ADD COLUMN IF NOT EXISTS strategy text NOT NULL DEFAULT 'server_relay',
  ADD COLUMN IF NOT EXISTS progress_bytes bigint NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS speed_bytes_per_sec bigint NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS actual_checksum_sha256 text,
  ADD COLUMN IF NOT EXISTS started_at timestamptz,
  ADD COLUMN IF NOT EXISTS paused_at timestamptz,
  ADD COLUMN IF NOT EXISTS completed_at timestamptz,
  ADD COLUMN IF NOT EXISTS error_code text;

ALTER TABLE file_transfer_chunks
...
ALTER TABLE ai_tool_calls
  ...
  ADD COLUMN IF NOT EXISTS confirmed_by uuid REFERENCES devices(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS confirmed_at timestamptz;
...
CREATE TABLE IF NOT EXISTS model_rule_profiles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  model_definition_id uuid REFERENCES model_definitions(id) ON DELETE CASCADE,
  model_key text NOT NULL,
  ...
  updated_at timestamptz NOT NULL DEFAULT now(),
  activated_at timestamptz,
  UNIQUE(user_id, model_key, profile_key)
);

CREATE INDEX IF NOT EXISTS model_rule_profiles_user_status_idx
  ON model_rule_profiles(user_id, model_key, status);
...
CREATE TABLE IF NOT EXISTS model_eval_cases (
  ...
  source_feedback_id uuid REFERENCES model_feedback_events(id) ON DELETE SET NULL,
  status text NOT NULL DEFAULT 'active',
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(user_id, model_key, case_key)
);

CREATE INDEX IF NOT EXISTS model_eval_cases_user_model_idx
  ON model_eval_cases(user_id, model_key, status);

CREATE TABLE IF NOT EXISTS model_rule_change_drafts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  model_definition_id uuid REFERENCES model_definitions(id) ON DELETE CASCADE,
  model_key text NOT NULL,
  proposed_version_id uuid REFERENCES model_versions(id) ON DELETE SET NULL,
  title text NOT NULL,
  summary text,
  risk_level text NOT NULL DEFAULT 'medium',
  proposed_profile_json jsonb NOT NULL DEFAULT '{}'::jsonb,
  status text NOT NULL DEFAULT 'pending_review',
  source text NOT NULL DEFAULT 'feedback_learning',
  created_at timestamptz NOT NULL DEFAULT now(),
  reviewed_at timestamptz
);

CREATE INDEX IF NOT EXISTS model_rule_change_drafts_user_status_idx
  ON model_rule_change_drafts(user_id, model_key, status, created_at DESC);
